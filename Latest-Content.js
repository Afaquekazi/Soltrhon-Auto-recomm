...
function displayContextSummaryInContainer(summaryData) {
    hideShimmerLoading();
    outputText.classList.remove('placeholder', 'error');
    ...
    if (summaryData.overview) {
        content += `OVERVIEW\n`;
        content += `Purpose: ${summaryData.overview.purpose || 'Not specified'}\n`;
        content += `Type: ${summaryData.overview.type || 'General discussion'}\n`;
        content += `Duration: ${summaryData.overview.duration_estimate || 'Unknown'}\n\n`;
    }
    ...
    if (summaryData.key_takeaways && summaryData.key_takeaways.length > 0) {
        ...
        summaryData.key_takeaways.forEach((takeaway, index) => {
    ...
    if (summaryData.topics && summaryData.topics.length > 0) {
        ...
        summaryData.topics.forEach(topic => {
            content += `• ${topic.topic} (${topic.importance} importance)\n`;
        });
        content += '\n';
    }
    ...
    if (summaryData.action_items && summaryData.action_items.length > 0) {
        ...
        summaryData.action_items.forEach(action => {
            content += `• ${action.action} (${action.priority} priority)\n`;
        });
    }
    
    outputText.textContent = content.trim();
}
...
document.addEventListener('contextmenu', async (e) => {
    ...
    if (selectedMode === 'smart_followups') {
        ...
        try {
            ...
            if (response && response.success && response.data) {
                ...
                // Call displayContextSummaryResults directly since we have the data
                if (response.data.summary) {
                    console.log('🎯 Displaying summary in main container...');
                    displayContextSummaryInContainer(response.data.summary);
                } else {
                    console.log('❌ No summary in response.data');
                    showError('No summary data received');
                }
            } else {
                console.log('=== RESPONSE FAILURE DEBUG ===');
                ...
                showError('Failed to generate context summary: ' + errorMsg);
            }

        } catch (error) {
            console.error('=== FULL CONTEXT SUMMARY ERROR ===');
            ...
            showError('Error analyzing conversation: ' + (error?.message || 'Unknown error'));
        }
    }
    ...
    if (selectedMode === 'context_summary') {
        const platform = detectAIPlatform();
        ...
        const conversation = extractConversation();
        console.log("=== DEBUG: CONTEXT SUMMARY CONVERSATION ===");
        console.log(conversation);
        console.log("=== END DEBUG ===");
        ...
        showShimmerLoading('Generating context summary...');
        solthronContainer.style.display = 'block';
        solthronContainer.style.pointerEvents = 'auto';
        const buttonRect = button.getBoundingClientRect();
        positionContainer(buttonRect);
        
        const creditCheck = await checkCredits('smart_enhancements'); // Using same credits as smart enhancements
        if (!creditCheck.success) {
            showError(creditCheck.message || "Please check your account status.");
            return;
        }
        ...
        try {
            ...
            if (response && response.success && response.data) {
                ...
                console.log('Has summary?', 'summary' in response.data);
                console.log('Summary value:', response.data.summary);
                console.log('=== END DATA DEBUG ===');
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    if (request.action === 'displayContextSummary') {
        displayContextSummaryResults(request.summary, request.platform, request.creditsUsed, request.creditsRemaining);
        return;
    }
